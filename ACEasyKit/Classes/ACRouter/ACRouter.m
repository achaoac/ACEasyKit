...
static NSString * const kACUrlSchemeJumpSel = @"openJumpUrl:extraParams:"; // 路由跳转需要实现的协议
static NSString * const kACUrlSchemeAction = @"action"; // 跳转之后要执行的动作
static NSString * const kACUrlSchemeActionParams = @"actionParams"; // 跳转之后要执行的动作传递的参数

@interface ACRouterConfig : NSObject
...
+ (void)configRouter:(NSString * _Nullable)urlScheme classMaps:(NSDictionary *)classMaps {
    [[ACRouterConfig sharedInstance] configRouter:urlScheme classMaps:classMaps];
}
...
+ (id)jump:(NSString *)urlStr params:(NSDictionary * _Nullable)params extraParams:(NSDictionary * _Nullable)extraParams {
    if (urlStr.length == 0 || ![urlStr hasPrefix:[ACRouterConfig sharedInstance].urlScheme]) {
        NSLog(@"ACRouter unavailable url - %@, please make sure %@ has prefix '%@' !!!", urlStr, urlStr, [ACRouterConfig sharedInstance].urlScheme);
        return nil;
    }
    NSURL *url = [NSURL URLWithString:urlStr];
    // 类名key
    NSString *classKey = [url host];
    NSString *className = [[ACRouterConfig sharedInstance] routerClassName:classKey];
    Class jumpClass = NSClassFromString(className);
    // 验证类是否合法
    if (!jumpClass) {
        NSLog(@"ACRouter can't jump to class %@ !!!", className);
        return nil;
    }
    if (![jumpClass conformsToProtocol:@protocol(ACRouterDelegate)]) {
        NSLog(@"ACRouter can't jump to class - %@, please implement protocol <ACRouterDelegate> !!!", className);
        return nil;
    }
    // 参数解析
    NSMutableDictionary *paramsDic = [NSMutableDictionary dictionaryWithDictionary:params];
    [paramsDic addEntriesFromDictionary:[ACRouter urlToDictionary:urlStr]];
    
    // 执行跳转动作
    SEL jumpSel = NSSelectorFromString(kACUrlSchemeJumpSel);
...
#pragma clang diagnostic pop
        ...
        // 执行跳转之后的额外操作
        NSString *actionSelName = [paramsDic objectForKey:kACUrlSchemeAction];
        if (jumpInstance && actionSelName) {
            SEL actionSel = NSSelectorFromString(actionSelName);
            id actionParams = [paramsDic objectForKey:kACUrlSchemeActionParams];
...
#pragma clang diagnostic pop
        ...
        NSLog(@"%@ cant't responds to selector %@ !!!", className, kACUrlSchemeJumpSel);
    }
    return nil;
}
...
+ (UINavigationController *)rootNavigationController {
    ...
    UIWindow *keyWindow = [ACRouter keyWindow];
    if (!keyWindow) {
...
+ (void)pushVC:(UIViewController *)viewController {
    UIViewController *vc = [ACRouter topViewController];
...
+ (NSString *)routerUrl:(NSString *)classKey {
    return [NSString stringWithFormat:@"%@://%@",[ACRouterConfig sharedInstance].urlScheme, classKey];
}
